            ...
            with st.spinner(t("qg_generating")):
                ...
                campaign = campaign_service.generate(
                    ...
                    target_audience=(
                        ...
                        "who want a clear solution to their problem"
                    ),
                    tone="Professional",
...
def _build_custom_product(
    ...
    return DiscoveryProduct(
        ...
        description=(
            ...
            "Commercial and market values are placeholders "
            "for testing."
        ),
    )
...
def _is_pro_user() -> bool:
    """Return True when the signed-in user has Pro access."""
    ...
    try:
        return SubscriptionService().is_pro(
            user_id
        )

    except Exception:
        # Never grant Pro access when subscription
        # verification is unavailable.
        return False


def _get_pro_display_price() -> str:
    """
    Return the YAffiliate Pro price for the selected language.

    Portuguese (Brazil) uses BRL.
    English, Spanish and Chinese use USD.
    """

    if get_language() == "pt_BR":
        return "R$49.90/month"
...
def _render_result(
    *,
    ...
    if not _is_pro_user():
        ...
        st.info(
            "🔒 Your complete marketing kit is ready. "
            "Upgrade to YAffiliate Pro to download the "
            "full campaign, including the SEO article, "
            "landing page, email sequence and Google Ads."
        )

        pro_price = _get_pro_display_price()

        if st.button(
            f"🚀 Upgrade to YAffiliate Pro — {pro_price}",
            type="primary",
        ...
        st.caption(
            "Your generated campaign is already saved. "
            "Upgrade to unlock the complete downloadable package."
        )
