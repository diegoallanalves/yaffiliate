...
def render() -> None:
    ...
    selected_campaign_id = st.selectbox(
        "Choose a campaign",
        campaign_ids,
        format_func=lambda campaign_id: _campaign_label(campaign_id, campaigns),
        key="campaign_history_selected_id",
    )

    selected_campaign = next(
        campaign for campaign in campaigns if campaign["id"] == selected_campaign_id
    )

    st.subheader("Overview")
    st.write("**Product:**", selected_campaign.get("product_name", ""))
    st.write("**Created:**", selected_campaign.get("created_at", ""))
    st.write("**User:**", selected_campaign.get("user_id", ""))
...
def _safe_file_name(value: str) -> str:
    ...
    return re.sub(r"_+", "_", cleaned).strip("_") or "yaffiliate_campaign"
