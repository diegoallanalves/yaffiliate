"""Campaign Generator page for Filtrify.

This page creates a coordinated affiliate-marketing campaign from one selected
product.

The current commercial version generates:

- one Search Engine Optimization article;
- one landing page;
- one email sequence;
- one Google Ads campaign;
- a campaign summary;
- individual plain-text downloads;
- one complete JSON campaign export;
- one complete ZIP campaign package.
...
def render() -> None:
    """Render the Filtrify Campaign Generator page."""
    ...
    form_column_1, form_column_2 = st.columns(
        2
    )
    ...
    with form_column_1:
        ...
        target_audience = st.text_area(
            "Target audience",
            value=(
                "People who want to learn Excel "
                "and improve their spreadsheet skills"
            ),
            placeholder=(
                "Describe the audience this campaign "
                "should target"
            ),
            height=110,
    ...
    _render_campaign_assets(
        campaign
    )

    _render_campaign_export(
        campaign
    )
...
def _render_campaign_summary(
    ...
    """Render the campaign-level summary."""
...
def _render_campaign_assets(
    campaign: CampaignPackage,
) -> None:
    """Render all generated campaign assets."""
...
def _render_seo_article(
    article: SEOArticle,
) -> None:
    """Render the generated Search Engine Optimization article."""
    ...
    st.download_button(
        ...
        data=article_text,
        file_name=(
            "filtrify_campaign_seo_article.txt"
        ),
        mime="text/plain",
        width="stretch",
...
def _render_landing_page(
    ...
    """Render the generated landing-page copy."""
    ...
    metric_column_1.metric(
        "Conversion score",
        (
            f"{landing_page.conversion_score:.1f}"
            "/100"
        ),
    )
    ...
    st.download_button(
        ...
        file_name=(
            "filtrify_campaign_landing_page.txt"
        ),
        mime="text/plain",
        width="stretch",
...
def _render_email_sequence(
    email_sequence: EmailSequence,
) -> None:
    """Render the generated email sequence."""
    ...
    metric_column_1.metric(
        "Emails",
        email_sequence.email_count,
    )

    metric_column_2.metric(
        "Estimated words",
        (
            f"{email_sequence.total_estimated_words:,}"
        ),
    )
    ...
    if email_sequence.strategy_summary:
        ...
        st.write(
            email_sequence.strategy_summary
        )
    ...
    st.download_button(
        ...
        file_name=(
            "filtrify_campaign_email_sequence.txt"
        ),
        mime="text/plain",
        width="stretch",
...
def _render_google_ads(
    ...
    """Render the generated Google Ads campaign."""
    ...
    headline_metric.metric(
        "Headlines",
        google_ads.headline_count,
    )

    description_metric.metric(
        "Descriptions",
        google_ads.description_count,
    )

    keyword_metric.metric(
        "Keywords",
        google_ads.keyword_count,
    )
    ...
    st.download_button(
        ...
        file_name=(
            "filtrify_campaign_google_ads.txt"
        ),
        mime="text/plain",
        width="stretch",
...
def _render_campaign_export(
    campaign: CampaignPackage,
) -> None:
    """Render complete JSON and ZIP campaign exports."""

    st.divider()

    st.subheader(
        "Export complete campaign"
    )

    st.caption(
        "Download the campaign as structured JSON "
        "or as a complete ZIP package containing "
        "DOCX, CSV, PDF, and JSON files."
    )

    try:
        campaign_json = (
            campaign_export_service.to_json(
                campaign
            )
        )

        campaign_zip = (
            zip_exporter.campaign_to_bytes(
                campaign
            )
        )
        ...
        st.error(
            "The campaign exports could not be prepared. "
            f"{error}"
        )
        return
    ...
    json_column, zip_column = st.columns(
        2
    )
    ...
    with json_column:
        st.download_button(
            ...
            data=campaign_json,
            file_name=(
                f"{safe_campaign_name}.json"
            ),
            mime="application/json",
            width="stretch",
            key=(
                "download_complete_campaign_json"
            ),
        )
    ...
    with zip_column:
        st.download_button(
            ...
            width="stretch",
            key=(
                "download_complete_campaign_zip"
            ),
        )

    st.caption(
        "The ZIP package includes the SEO article, "
        "landing page, email sequence, campaign summary, "
        "Google Ads CSV, JSON data, and PDF summary."
    )
...
def _build_article_text(
    ...
    """Convert a structured SEO article into plain text."""
...
def _build_landing_page_text(
    landing_page: LandingPage,
) -> str:
    """Convert a structured landing page into plain text."""
    ...
    blocks = [
        ...
        (
            "Meta Description\n"
            f"{landing_page.meta_description}"
        ),
        "",
    ...
    blocks.extend(
        [
            ...
            (
                ...
                f"{landing_page.final_cta_button}"
            ),
        ]
    )
...
def _sanitize_download_file_name(
    value: str,
) -> str:
    """Convert a campaign name into a safe download file name."""
...
def _validate_campaign_fields(
    *,
    ...
    """Return campaign fields that do not contain a value."""
